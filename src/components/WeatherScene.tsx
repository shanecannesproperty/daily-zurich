// Animated weather hero. A single inline SVG (viewBox 800×200) whose contents
...
//
// The artboard is a wide 4:1 so it matches the full-bleed hero strip: with
// `xMidYMax slice` the scene fills the panel edge-to-edge, anchored to the
// horizon, cropping only empty upper sky instead of zooming the clouds out of
// proportion. Everything is deterministic (no Math.random) so the
...
// from the index so SSR and client agree. Spread across the full 800 artboard so
// precipitation covers the whole hero, not just its centre.
function Precip({ kind }: { kind: "rain" | "snow" }) {
  const count = kind === "rain" ? 44 : 36;
  const drops = Array.from({ length: count }, (_, i) => {
    const x = ((i * 53) % 100) * 8; // spread across 0–800
...
// spread across the wide artboard and kept in the lower band that stays visible
// after the top-sky crop.
function Stars() {
  const pts = [
    [60, 52],
    [130, 40],
...
// Sits in the right third of the wide artboard.
function Luminary({ isDay }: { isDay: boolean }) {
  if (!isDay) {
    return (
      <g className="wx-pulse" transform="translate(560 64)">
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Cloud cover, layered by how heavy the scene is. Spread into a bank
            across the wide artboard so it reads in proportion on the hero. */}
        {scene === "partly" && (
          <g className="wx-cloud">
            <Cloud cx={300} cy={96} scale={1} fill={cloud} />
          </g>
        )}
        {(scene === "cloudy" ||
          scene === "rain" ||
          ...
          <>
            <g className="wx-cloud-2">
              <Cloud cx={430} cy={92} scale={1} fill={cloud} opacity={0.95} />
            </g>
            <g className="wx-cloud">
              <Cloud cx={190} cy={100} scale={1.15} fill={cloud} />
            </g>
            <g className="wx-cloud-2">
              <Cloud cx={650} cy={104} scale={1.2} fill={cloud} opacity={0.95} />
            </g>
          </>
        )}
        ...
        {scene === "storm" && <rect className="wx-flash" width="800" height="200" fill="#fffbe6" />}
