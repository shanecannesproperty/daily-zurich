...
import { getMostRead, type MostReadArticle } from "@/lib/articles.functions";
import { CATEGORY_LABELS } from "@/lib/schema";

export function MostRead() {
  const fetchMostRead = useServerFn(getMostRead);
  const { data } = useQuery({
    queryKey: ["most-read"],
  ...
  const articles: MostReadArticle[] = data?.articles ?? [];
  const fallback = data?.fallback ?? false;
  if (articles.length === 0) return null;

  const heading = fallback ? "Latest" : "Most Read";

  return (
    <section className="border-t border-[var(--ink)] pt-5">
      <h2 className="kicker mb-4">{heading}</h2>
